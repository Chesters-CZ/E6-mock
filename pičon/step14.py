...
def get_user_blips(user_id: int):
    ...
    for blip in blips:
        try:
            blips_out.append([blip.findNext("div", class_="post-time").findChildren()[0]["href"][7:],
                              user_id,
                              blip.findNext("div", class_="styled-dtext").text,
                              ""])
...
print("\nWARNING: SCRIPT APPENDS TO step14\\blip_text_post.csv AND WILL NOT REPLACE EXISTING DATA\n")

discard = []
already_scraped = []

load_csv("step14\\blip_text_post.csv", discard, already_scraped, False)

already_scraped_ids = [blip[1] for blip in already_scraped]
del already_scraped
...
with open("D:\\velký dbs fily\\step14\\blip_text_post.csv", mode="a", encoding="utf-8", newline="") as blips_out:
    ...
    for i, user in enumerate(user_ids_to_be_scraped):
        ...
        try:
            comments = get_user_blips(user[0])
